...
func runServer(cmd *cobra.Command, args []string) error {
	...
	predicatoInstance, embedderClient, err := initializePredicato(cmd, cfg)
	if err != nil {
...
func initializePredicato(cmd *cobra.Command, cfg *config.Config) (predicato.Predicato, embedder.Client, error) {
	...
	case "ladybug":
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to create ladybug driver: %w", err)
		}

	case "falkordb":
		// FalkorDB support would be implemented here
		return nil, nil, fmt.Errorf("FalkorDB driver not yet implemented")
	default:
	...
	if useGLiNER2 {
		...
		if err := ensureGLiNER2Server(endpoint); err != nil {
			return nil, nil, fmt.Errorf("failed to ensure GLiNER2 server: %w", err)
		}
		...
		if err != nil {
			return nil, nil, fmt.Errorf("failed to create GLiNER2 client: %w", err)
		}
	...
	if nlProcessor == nil && defaultModel.APIKey != "" {
		...
		case "openai":
			...
			if err != nil {
				return nil, nil, fmt.Errorf("failed to create NLP client: %w", err)
			}
			...
			if err != nil {
				return nil, nil, fmt.Errorf("failed to create retry client: %w", err)
			}
			...
			if err := os.MkdirAll(trackingPath, 0755); err != nil {
				return nil, nil, fmt.Errorf("failed to create telemetry directory: %w", err)
			}
		...
		default:
			return nil, nil, fmt.Errorf("unsupported NLP provider: %s", defaultModel.Provider)
		}
	...
	if cfg.FactStore.ConnectionString != "" {
		...
		if err := os.MkdirAll(dataPath, 0755); err != nil {
			return nil, nil, fmt.Errorf("failed to create factstore data directory: %w", err)
		}
	...
	return client, embedderClient, nil
}
